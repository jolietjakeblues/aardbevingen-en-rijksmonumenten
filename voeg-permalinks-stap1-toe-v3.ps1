...
# 2. Permalinkfuncties vóór initMap().
...
# 4. Na PDOK-laag juiste basislaag activeren.
...
# 5. Layer-control uitbreiden met listeners.
...
# 6. Wat ligt hier?-klik in permalink bewaren.
$clickPattern = '(?m)^      showWhatIsHere\(e\.latlng\.lat, e\.latlng\.lng\);$'
$clickReplacement = @'
      state.permalinkWhatIsHere = {
        lat: e.latlng.lat,
        lon: e.latlng.lng
...
# 7. Permalink herstellen tijdens initialisatie.
$initPattern = '(?m)^  initNlogControls\(\);\r?\n  initWhatIsHere\(\);\r?\n  loadNlogLayers\(\);\r?\n  loadEarthquakes\(\);$'
$initReplacement = @'
  initNlogControls();
  initWhatIsHere();
...
$content = [regex]::Replace(
    ...
    [System.Text.RegularExpressions.MatchEvaluator]{
        param($m)
        $m.Groups[1].Value.TrimEnd("`r","`n") + $pdokAppend + "`r`n"
    },
    1
)
...
$content = [regex]::Replace(
    ...
    [System.Text.RegularExpressions.MatchEvaluator]{
        param($m)
        $m.Groups[1].Value.TrimEnd("`r","`n") + $controlAppend + "`r`n"
    },
    1
)

$content = [regex]::Replace($content, $clickPattern, $clickReplacement, 1)
